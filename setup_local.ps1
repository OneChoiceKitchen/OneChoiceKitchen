<#
.SYNOPSIS
  OneChoiceKitchen — Local Development Orchestrator v5.0 (setup_local.ps1)
  Fully self-healing. Zero manual steps. Docker-free (but Docker-aware).
  For PRODUCTION deployment, use: .\setup_deployment.ps1 -Target docker|vercel|vps
...
function Invoke-DatabaseSetup {
    ...
    if (Test-Path "$Root\prisma\seeds\seed_dev_data.ts") {
        dim 'Seeding development data (restaurants, branches, menu items, inventory)...'
...
function Start-All {
    ...
    # ── Open Prisma Studio (always, or when -OpenDB) ─────────────────────────
    if (-not $NoBrowser -and $OpenDB) {
        Write-Host ''
        cyn 'Opening Prisma Studio (database browser)...'
