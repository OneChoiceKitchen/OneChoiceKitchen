...
db.serialize(() => {
  pages.forEach(page => {
    db.get('SELECT slug FROM StaticPage WHERE slug = ?', [page.slug], (err, row) => {
      if (err) {
        console.error(err.message);
        return;
      }
      if (!row) {
        // use nanoid for id like uuid
        const id = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
        const now = new Date().toISOString();
        db.run(
          'INSERT INTO StaticPage (id, slug, title, content, section, isPublished, portals, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [id, page.slug, page.title, page.content, page.section, 1, page.portals, now, now],
          (err) => {
            if (err) console.error(err.message);
            else console.log(`Created page: ${page.title}`);
          }
        );
      }
    });
