  ...
  for (const page of pages) {
    try {
      const res = await db.execute({
        sql: 'SELECT slug FROM StaticPage WHERE slug = ?',
        args: [page.slug]
      });
      if (res.rows.length === 0) {
        ...
        await db.execute({
          sql: 'INSERT INTO StaticPage (id, slug, title, content, section, isActive, portals, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
          args: [id, page.slug, page.title, page.content, page.section, 1, page.portals, now, now]
        });
