...
export default function BranchLandingPage() {
  ...
  const handleNavigation = (path: string) => {
    // In mobile app, map the path to a tab (menu -> home/menu tab, reservations -> dinein, tiffin -> tiffin-checkout)
    let tab = path;
    if (path === 'reservations') tab = 'dinein';
    if (path === 'tiffin') tab = 'tiffin-checkout';
    if (path === 'menu') tab = 'home'; // Or whatever tab renders the menu

    let url = `/?tab=${tab}&branchId=${branchId}`;
    if (tableId) {
      url += `&tableId=${tableId}`;
    }
    router.push(url);
